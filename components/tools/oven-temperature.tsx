...
export function OvenTemperatureTool() {
  const ui = useToolUi("oven-temperature");
  const tempUi = useToolUi("temperature");
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        {(["c", "f"] as const).map((u) => (
          <button key={u} type="button" className={`btn-preset flex-1 ${active === u ? "btn-preset-active" : ""}`} onClick={() => setActive(u)}>
            {u === "c" ? tempUi.celsius : tempUi.fahrenheit}
          </button>
        ))}
      </div>
      <label className="tool-panel block">
        <span className="text-sm font-medium">{ui.temperature}</span>
      ...
      {result && (
        <div className="result-card space-y-2">
          ...
          <p>{ui.gasMark} <strong>{result.gasMark}</strong></p>
        </div>
      )}
      <div className="tool-panel overflow-x-auto">
        <p className="mb-2 text-sm font-medium">{ui.gasReference}</p>
        <table className="w-full text-sm">
          <thead><tr className="text-left text-[var(--muted)]"><th className="pb-1">{ui.gas}</th><th>°C</th><th>°F</th></tr></thead>
          <tbody>
