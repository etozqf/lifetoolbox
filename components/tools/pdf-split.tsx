...
export function PdfSplitTool() {
  ...
  const split = async () => {
    ...
    try {
      ...
      setStatus(`Extracted pages ${start}–${end}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <label className="tool-panel block"><span className="text-sm font-medium">From page</span>
          <input type="number" min="1" className="tool-input mt-2" value={start} onChange={(e) => setStart(e.target.value)} /></label>
        <label className="tool-panel block"><span className="text-sm font-medium">To page</span>
          <input type="number" min="1" className="tool-input mt-2" value={end} onChange={(e) => setEnd(e.target.value)} /></label>
      </div>
      <button type="button" className="btn-primary w-full" onClick={split} disabled={!file}>Extract & Download</button>
      {status && <p className="text-sm">{status}</p>}
      <p className="text-xs text-[var(--muted)]">Processed locally in your browser.</p>
    </div>
  );
}
