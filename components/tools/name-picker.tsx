...
export function NamePickerTool() {
  ...
  const pick = () => {
    ...
    else { setError(out.error); setWinner(""); }
  };
  return (
    <div className="space-y-4">
      <label className="tool-panel block">
        <span className="text-sm font-medium">Names (one per line)</span>
        <textarea className="tool-input mt-2 min-h-[160px] font-sans" value={names} onChange={(e) => setNames(e.target.value)} />
      </label>
      <button type="button" className="btn-primary w-full" onClick={pick}>Pick a name</button>
      {error && <p className="text-red-500">{error}</p>}
      {winner && (
        <div className="result-card text-center">
          <p className="text-sm text-[var(--muted)]">The winner is</p>
