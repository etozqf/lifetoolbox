    ...
    <div className="space-y-4">
      ...
      {result && (
        <div className="result-card space-y-2">
          <p className="result-value text-2xl">{result.liters.toFixed(1)} L/day</p>
          <p className="text-sm text-[var(--muted)]">{Math.round(result.ml)} ml · {result.cups.toFixed(1)} cups · {Math.round(result.oz)} fl oz</p>
        </div>
      )}
    </div>
  );
